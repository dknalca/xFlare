...
- **Techo del teal, tanda 3.** El mapeo lineal de `renderUserTrace` ya era
  correcto (`[posLo, posHi] -> [yb, yt]`); lo que fallaba es que con la friccion
  vieja el plato no LLEGABA a `posHi` con un gesto de trackpad. Afinado:
  `frictionPerSecond` 2.5 -> 1.8, `scrollGain` 0.26 -> 0.40, `keyImpulse`
  1.6 -> 2.2, sensibilidad por defecto 0.5 -> 1.0.
