...
- **Aguja del rail = linea de la autopista.** La aguja del rail izquierdo ya no
  ...
  linea del usuario -o del fantasma- bajo el cabezal en el `HighwayFrame`. Y la
  onda del sample ocupa **exactamente la banda vertical de la curva**
  (`HighwayGeometry.curveBand`), no la zona entera: el rango de alto del sample
  y el del movimiento son el mismo. "Donde estas en el sample" = "donde estas en
  la autopista".

**Iterado (2026-09-02, feedback imagen #15).**
- **El movimiento llena la autopista.** `HighwayGeometry.patternFill` pasa a
  `1.0` (antes 2/3, ADR-041): la curva del patron ocupa de arriba abajo, sin el
  tercio vacio que se leia como un techo. `curveInset` 28 -> 12. El plato aun
  puede pasarse del pico (`posHi`, hasta el final del sample); esa traza extra
  se sale por el borde superior y la recorta el `SKCropNode`. El mapeo de AUDIO
  (`scratchPatternTopFraction` = 2/3 en `normalizedPosition`) no cambia.
