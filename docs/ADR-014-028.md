...
- Alterna cada `crBars` compases. `crBars` se redondea a un múltiplo entero de la
  longitud del patrón (≈ 2 compases) para que cada `listen` toque la **misma**
  frase completa.

`LivePracticeView`: botón en la barra superior (Llamada y respuesta / Escucha… /
Tu turno) y el gain del scratch pasa a un `.onChange(of: session.faderClosed)`,
así el mute funciona tanto si cierra el fader el usuario como el fantasma.
...
- ⚠️ Sin comparación automática llamada↔respuesta todavía (eso es `XFAnalysis`).
  De momento el oído es el juez. El número de compases no es configurable por UI.
...
2. **`TempoAnalyzer`** (XFApp, offline, sin dependencias): envolvente de onset
   (flujo de energia RMS) → autocorrelacion en 70-170 BPM para el periodo de
   negra (con sesgo suave anti-octava) → si el bucle dura ~un numero entero de
   negras se cuadra el BPM a eso → correlacion con tren de pulsos para la fase
   del primer golpe. Devuelve `(bpm, phaseFrames, beatsInLoop)`.

   Al cargar la practica: se analiza la instrumental; si sale, se **rota** el PCM
   para que empiece en el "1" (`phaseFrames`), se carga con ese `nativeBPM`, la
   sesion pasa a ese tempo (`session.setBPM`) y el reloj se pone a 0
   (`resyncClock`) justo al arrancar el audio. Asi `tick 0` = primer golpe del
   bucle y la rejilla (autopista y tira) cae sobre los golpes.

**Consecuencias.**
- ✅ La rejilla sigue el ritmo real de la cancion, no un tempo asumido.
- ✅ La practica arranca al tempo detectado (no siempre 80).
- ✅ Todo XFApp, nada sellado. `TempoAnalyzer` es puro y testeable (tren de
   clicks sintetico).
- ⚠️ Autocorrelacion simple: puede errar la octava en material ambiguo; el
   ajuste al bucle lo resuelve en la mayoria de loops limpios. Sin deteccion de
   compas (asume 4/4) ni de cambios de tempo.
