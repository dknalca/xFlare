...
4. **Redibujo al vsync + onda pre-renderizada (addendum).** El primer intento
   (imagen 1:1 + `Timer` de 60 Hz para `needsDisplay`) seguia temblando: el
   `Timer` no esta en fase con el refresco → frames dobles/perdidos. Ahora las
   dos tiras heredan de `DisplayLinkView` (redibujo enganchado a `CVDisplayLink`,
   como hace `SKView` con la autopista) y la onda se **renderiza a un `CGImage`
   una sola vez** (`WaveformImage`, ancho natural del bucle); cada frame solo se
   blitea 3 veces desplazada (instrumental, que hace loop) o recortada por
   `progress` (sample). Coste por frame ~nulo → scroll fino.
   El sample (`WaveformStripView`) usa la misma `WaveformColored` → color por
   frecuencia tambien abajo.
