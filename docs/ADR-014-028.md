...
- **Aguja del rail = linea de la autopista.** La aguja del rail izquierdo ya no
  ...
  onda del sample ocupa exactamente la franja vertical de la autopista (no se
  mete bajo la tira). "Donde estas en el sample" = "donde estas en la autopista".
...
se paran aparte, desde la vista (`engine.setTransport(playing: !frozen)`), porque
el reloj del motor es otro. Indicador "CONGELADO" en la barra superior.
**Consecuencias.**
- ✅ Repetir un trozo concreto sin manejar el transporte a mano.
- ⚠️ Congelado, el motor de audio de la instrumental esta en pausa pero el
  `PracticeSession` sigue latiendo a 60 Hz (solo mueve el plato): coste nulo.
