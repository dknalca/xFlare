...
- **← / →** (fase): `PracticeSession.nudgePhase(±ppq/48)` desplaza la rejilla
  respecto a la base (offset al reloj). Ajuste fino manual cuando la deteccion
  no cae exacta en los golpes.
- **Deteccion de fase mejorada.** El comb de fase de `TempoAnalyzer` ahora
  interpola lineal la envolvente (sub-frame) y refina el pico por parabola;
  ademas suma **medio hop** a `phaseFrames` (el flux marca el bloque donde SUBE
  la energia; el golpe real cae hacia el centro). La rejilla iba adelantada.
- **Techo del teal, tanda 3-4.** El afinado (friccion 2.5->1.8, scrollGain
  0.26->0.40, keyImpulse 1.6->2.2, sensibilidad 0.5->1.0) no bastaba: un gesto
  realista llega a ~n=1,1 y el mapeo `[posLo, posLo+1.5·span]->[yb,yt]` ponia
  eso a ~2/3. Cambio de modelo: `renderUserTrace` mapea el **rango propio del
  patron** `[lo, lo+span]` (n=0..1) a **toda** la banda `[yb, yt]`, SIN techo
  (n>1 se sale por arriba y lo recorta el `SKCropNode` = "infinito hacia
  adelante"). `posHi` sube a `posLo + 2,5·span` para dar recorrido. La onda
  FANTASMA (mas baja, escalada con la amplitud) y la traza ya no coinciden en
  altura: se sigue la FORMA/ritmo, no el pixel. La aguja del rail si se queda
  en `[yb, yt]` (el rail es el sample).
