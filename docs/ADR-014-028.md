...
- La **traza del usuario** tampoco tiene techo: `patternFill` fijo a `2/3` y
  `posHi = posLo + patternSpan / (2/3)` (n_max = 1,5) -> al empujar el plato a
  tope la traza llega justo al borde superior. El mapeo de AUDIO
  (`normalizedPosition`) usa la constante `scratchPatternTopFraction`, sin
  amplitud. `PracticeSession` no sabe nada de amplitud.
