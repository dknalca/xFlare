...
**Iterado (2026-09-02, feedback imagen #16). Slider de amplitud.**
- El rail izquierdo vuelve a ocupar **toda** la franja de la autopista (`0…hh`):
  es el sample ENTERO. No se estira ni encoge con nada.
- El **movimiento** (curva del patron + traza) tiene un slider "Amplitud" en el
  panel derecho: `HighwayGeometry.patternFill` = amplitud. El principio del
  movimiento SIEMPRE es el borde inferior (inicio del sample); la amplitud
  cambia solo hasta donde llega el PICO. `2/3` por defecto (el pico a 2/3 del
  "Ahh"); `1.0` = arriba del todo.
- La amplitud tambien mueve el mapeo de AUDIO: `PracticeSession.amplitude`
  (antes la constante `scratchPatternTopFraction`) escala `normalizedPosition` /
  `normalizedVelocity` y recalcula `posHi = posLo + patternSpan·(1/amp + 0.15)`
  (el +0.15 deja aire para no toparse con un muro). "Donde acaba el movimiento
  respecto al sample" = la amplitud.
- `curveInset`/`laneHeight` a 8: el movimiento arranca casi pegado al borde
  inferior, como el rail.
- (Anula la nota de la imagen #15: `patternFill` ya no es fijo `1.0`.)
