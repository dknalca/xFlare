...
- **Botones de rejilla que no se pulsaban.** El panel a 136 px aplastaba los
  chips por debajo de su tamano -> target de toque ~0. Panel a 176 px, chips de
  30x22 con `.fixedSize()` y borde. Paso `ppq/8`.
- **"Llamada y respuesta" -> "Repite conmigo"**, y la seccion mas compacta (una
  fila: boton + `− n +`).
