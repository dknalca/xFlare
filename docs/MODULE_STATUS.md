...
| `XFRender` | 2 | WIP | — | 29 verdes | `HighwayLayout` (B7.3+B7.4): geometria pura de la autopista — fantasma + capa de usuario teñida por `HitLevel` + cabeza de lectura, loop con el patron, anti-deriva. `ScopeLayout` (B7.5): scope circular del plato (Lissajous reconstruido de `position`/`confidence`, degradado con señal sucia). `HighwayScene`/`HighwayView` y `ScopeScene`/`ScopeView` (SpriteKit/SwiftUI delgadas) sincronizan al reloj de AUDIO via `update(_:)`. Falta golden SVG (B7.6). fps en la maquina pendiente (B7.2b) |
| `XFTestKit` | — | TODO | — | — | |
| `XFApp` | 3 | TODO | — | — | |
