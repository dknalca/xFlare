...
| `XFEngine` | 1 | WIP | — | 29 verdes | `SessionMachine` (B9.1): maquina de estados de la sesion. `BPMLadder` (B9.2): escalera de BPM adaptativa (3 aprobados suben, 2 fallos bajan). `UnlockRule`/`UnlockTracker` (B9.3): desbloqueo por N compases buenos seguidos, no por media. Todo struct valor que avanza por eventos como `Transport`. Falta cablear los tres en un facade de sesion y sellar (B9.4) |
