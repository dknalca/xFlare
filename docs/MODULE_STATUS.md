...
| `XFRender` | 2 | WIP | — | 19 verdes | `HighwayLayout` (B7.3+B7.4): geometria pura de la autopista — fantasma (curva del disco, marcas de fader, carril), capa de usuario partida en `TintedPolyline` por `HitLevel` y marcas de click teñidas, cabeza de lectura al 30%. Loop con el patron, testeable sin SpriteKit (anti-deriva `frame(T)==frame(T+L)`). `HighwayScene`/`HighwayView` (SpriteKit/SwiftUI delgadas) sincronizan al reloj de AUDIO via `update(_:)`. Falta scope Lissajous (B7.5), golden SVG (B7.6). fps en la maquina pendiente (B7.2b) |
| `XFTestKit` | — | TODO | — | — | |
| `XFApp` | 3 | TODO | — | — | |
