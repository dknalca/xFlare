...
| `XFPersistence` | 1 | WIP | — | 10 verdes | `XFDatabase` + `Schema` (B10.1): abre el SQLite con GRDB 6.x y aplica el migrador. Migracion `v1` con las 10 tablas del bloque B10 (practiceSession, attempt+attemptEvent, exerciseProgress, variantUnlock, exerciseMastery, reviewSchedule, deviceCalibration, practiceDay, setting). Falta el codigo de consulta tipado (B10.2-B10.8) |
