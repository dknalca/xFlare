...
| `XFRender` | 2 | WIP | — | 11 verdes | `HighwayLayout` (B7.3): geometria pura de la autopista (curva del disco, marcas de fader, carril, cabeza de lectura), loop con el patron, testeable sin SpriteKit — foco anti-deriva (`frame(T) == frame(T+L)`). `HighwayScene`/`HighwayView` (SpriteKit/SwiftUI delgadas) sincronizan al reloj de AUDIO via `update(_:)`. Falta capa de usuario + tenido (B7.4), scope Lissajous (B7.5), golden SVG (B7.6). fps en la maquina pendiente (B7.2b) |
| `XFTestKit` | — | TODO | — | — | |
| `XFApp` | 3 | TODO | — | — | |
