...
| `XFEngine` | 1 | WIP | — | 19 verdes | `SessionMachine` (B9.1): maquina de estados de la sesion (warmup/series/rest/boss/results, `docs/CURRICULUM.md` §3). `BPMLadder` (B9.2): escalera de BPM adaptativa (3 aprobados suben, 2 fallos bajan). Ambos struct valor que avanzan por eventos como `Transport`. Falta desbloqueo por compases seguidos (B9.3) |
