...
- [x] **B11.3** Pantalla de practica (la autopista) `XFApp`
      ...
      - Iterado (2026-09-01, feedback): **ruido de fondo** = zumbido de DC del `xf_player` parado -> puerta por velocidad (`xf_player_set_speed_gate`, el motor la pone a 0,12 en el scratch). Fader cerrado: se quita el overlay negro a pantalla completa; solo el tramo de la traza del usuario se pinta apagado (`.miss`), la onda NO. Zoom out de la onda (0,16 -> 0,5). Aguja de la onda en `needleFraction` (0,30), no en el centro. **La onda y la autopista comparten posicion**: el cabezal se ancla a `PracticeSession.normalizedPosition` cada fotograma (abajo = inicio del sample, arriba = final); nunca se pinta onda fuera del sample. **Miniatura TTM** rehecha: curva partida donde el fader cierra (hueco = mute) + circulo por transicion, sin barra inferior. `ex-l1-baby` 70->50 BPM.
