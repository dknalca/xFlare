...
- [x] **B11.3** Pantalla de practica (la autopista) `XFApp`
      ...
      - Anadido (2026-09-01): mas feedback de la practica. Fader cerrado -> la autopista se **oscurece** (ya cortaba el sonido). **Tira de forma de onda** del sample de scratch (`WaveformEnvelope` puro + `WaveformStripView` estilo DVS: la onda se desplaza bajo una aguja fija siguiendo `xf_engine_scratch_playhead`). `ex-l1-baby` arranca mas lento (`startBpm` 70->50, escalera 50..90). 9 tests (4 envolvente + 1 cabezal + los de baby actualizados).
