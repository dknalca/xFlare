...
- [x] **B11.3** Pantalla de practica (la autopista) `XFApp`
      ...
      - Iterado (2026-09-01, feedback): **ruido de fondo** = zumbido de DC del `xf_player` parado -> puerta por velocidad (`xf_player_set_speed_gate`, el motor la pone a 0,12 en el scratch). Fader cerrado: se quita el overlay negro a pantalla completa; en su lugar el tramo de la traza del usuario se pinta apagado (`.miss`) y la tira de onda baja opacidad. Zoom out de la onda (0,16 -> 0,5). Sample rebobinado a 0 al entrar (`xf_engine_seek_scratch`). 10 tests nuevos en total (envolvente, cabezal, puerta por velocidad, baby).
